...
- 同采样率换曲时保持现有输出流，不暂停、不切换设备；只有采样率实际变化或软件首次接管播放时才重建外置 DAC 音频流。
- 外置 DAC 变采样率时会在暂停状态下暂时释放到内置输出，待目标 DAC 空闲并锁定后再切回；“重新匹配”和“恢复声音”也会执行这套完整路由重置。
- WALKMAN 等外置 DAC 会采用静音的活动路由交接：Apple Music 全程保持播放，在备用输出稳定运行同一条音频流后直接切回 DAC，不再在中途暂停/恢复；这与手动切换设备的有效路径一致且不会从内置扬声器漏音。
- “DAC 锁定等待”会作为硬件的最短稳定时间执行，避免设备只更新了标称采样率、USB 音频端点却尚未就绪。
